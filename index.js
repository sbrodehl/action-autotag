  ...
  try {
    ...
    if (!matches)
      return core.setFailed(
        `No match was found for the regex '${regex.toString()}'.`,
      );

    let capture_group = core
      .getInput("capture_group", { required: false })
      .trim();
    if (capture_group.length === 0) capture_group = 0;
    if (capture_group >= matches.length)
      return core.setFailed(
        `Unable to find capture_group '${capture_group}' in '${matches.length}' matches.`,
      );
    ...
    if (message.length === 0 && tags && tags.data.length > 0) {
      ...
      let changelog = await octokit.rest.repos.compareCommitsWithBasehead({
        owner,
        repo,
        basehead,
      });
    ...
    core.warning(
      `Reference ${reference.data.ref} is now available at ${reference.data.url}`,
    );
